	...
	public CustomexProtocalCodecFactory() {
		...
		decoder = new CumulativeProtocolDecoder() {
			...
			protected boolean doDecode(IoSession session, IoBuffer in,
				...
				// Реализуйте пользовательский анализ 
                                //с подклассом TcpRecvParser в соответствии с настройками
				Class<?> zz = Class.forName(handler);
                                //Исправлено: 20.04.2021 ////////////////////////////////////////////////////////
				TcpRecvParser parser = (TcpRecvParser) zz.getDeclaredConstructor().newInstance();//zz.newInstance();
                                /////////////////////////////////////////////////////////////////////////////////
				ret = parser.parseRevContent(bs);
				result = (Boolean) ret[0];

				// Оставьте оставшихся персонажей
				if (result == true) {

					int readLength = (Integer) ret[1];
					// System.out.println("readLength:" + readLength);
					// System.out.println("bs.length:" + bs.length);
					// Когда запрашивающая сторона заключила субподряд, необходимо принять это решение
